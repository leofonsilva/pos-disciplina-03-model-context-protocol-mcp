...
|--------|-------------|----------|------------------------------|
| admin  | leofonsilva | 123123   | read, create, update, delete |
| member | tamyresende | 1234     | read only                    |
...
curl -X POST http://localhost:9999/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "leofonsilva", "password": "123123"}'
```
...
curl -X POST http://localhost:9999/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "tamyresende", "password": "1234"}'
```
